...
export function CostForm({
  ...
  const isDerivedOrigin = Boolean(existingCost?.originType && existingCost.originId);

  useEffect(() => {
    // A cost generated by another module (Payable, Employee allocation)
    // is managed exclusively from its source — redirect away instead of
    // exposing the manual edit/delete form for it, even when this route
    // is opened directly by URL.
    if (!existingCost?.originType || !existingCost.originId) return;
    if (existingCost.originType === "payable") {
      router.replace(`/financeiro/contas-a-pagar/${existingCost.originId}`);
    } else if (existingCost.originType === "employee-period-allocation") {
      const target = resolveAllocationPeriodRoute(existingCost.originId);
      router.replace(target ?? `/obras/${projectId}/custos`);
    }
  }, [existingCost, router, projectId]);
  ...
  function handleDelete() {
    ...
    if (!confirmed) return;
    deleteManualProjectCost(existingCost.id);
    router.push(`/obras/${projectId}/custos`);
  }

  if (isEditing && existingCost === undefined) return null;
  if (isDerivedOrigin) return null;
