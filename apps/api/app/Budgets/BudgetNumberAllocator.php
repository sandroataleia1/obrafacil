...
/**
 * BUDGET-API-01. Allocates the next sequential `number` for a Company,
 * race-safe under real concurrent PostgreSQL transactions via
 * `SELECT ... FOR UPDATE` row locking on `budget_sequences`. Mirrors
 * `App\ServiceOrders\ServiceOrderNumberAllocator` exactly.
 *
 * MUST be called from within the caller's own transaction — that is what
 * makes a later failure (an invalid item, a discount check, anything)
 * roll back the `next_number` increment along with everything else, so a
 * failed create never burns a number.
 */
class BudgetNumberAllocator
{
    public function allocate(string $companyId): int
    {
        $row = DB::table('budget_sequences')
    ...
    private function ensureSequenceRowExists(string $companyId): void
    {
        try {
            DB::transaction(function () use ($companyId) {
                DB::table('budget_sequences')->insert([
