 ...
 *
 * "subtotal" alone is deliberately never used anywhere in this domain
 * (BUDGET-API-01A §6) — with two distinct subtotal concepts
 * (`sale_subtotal`/`cost_subtotal`), an unqualified name is ambiguous.
 */
final class BudgetCalculator
{
     ...
     */
    public static function saleSubtotal(iterable $lineTotals): string
    {
        $sum = '0.00';
    ...
    public static function marginAmount(string $saleSubtotal, ?string $costSubtotal): ?string
    {
        if ($costSubtotal === null) {
            return null;
        }

        return Money::subtract($saleSubtotal, $costSubtotal);
    }
    ...
    public static function total(string $saleSubtotal, string $discountAmount): string
    {
        return Money::subtract($saleSubtotal, $discountAmount);
    }
}
