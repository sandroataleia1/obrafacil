...
/**
 * POST /api/v1/catalog-items — §24. `active` defaults to `true` when
 * omitted (handled by the model's `$attributes` default, not here).
 */
class StoreCatalogItemRequest extends FormRequest
{
    ...
    {
        return [
            // §23: hostile/foreign fields are rejected, never silently dropped.
            'id' => ['prohibited'],
    ...
    /**
     * §8/§9: a pre-check for a clean 422 in the common case — the real
     * partial unique index (catalog_items_company_code_unique) remains
     * the authoritative, race-safe backstop regardless of this check.
     */
    private function validateCodeIsUnique(ValidatorContract $validator): void
    {
        ...
        $exists = CatalogItem::query()->whereRaw('LOWER(code) = ?', [Str::lower($code)])->exists();
    ...
    /**
     * §26: trim, and an empty string means "not set" — never a distinct
     * "" value alongside null.
     */
